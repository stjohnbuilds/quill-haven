/* ───────────────────────────────────────────────
   Shared confirm dialog — ONE styled popup used by
   the home screen and every app (no duplicates).
   Usage:
     qhConfirm({
...
(function () {
  ...
  function injectStyle() {
    ...
    s.textContent =
      '.qhc-overlay{position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.28);' +
      'display:flex;align-items:center;justify-content:center;padding:20px;' +
      'backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px);animation:qhcFade .15s ease;' +
      'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;}' +
      '.qhc-box{background:var(--panel-bg,#fff);border:1px solid var(--card-border,rgba(0,0,0,0.1));' +
      ...
      '.qhc-title{font-size:16px;font-weight:600;color:var(--text-strong,#333);}' +
      '.qhc-msg{font-size:13px;color:var(--text-muted,#888);margin-top:8px;line-height:1.5;}' +
      ...
      '.qhc-cancel{background:var(--row-hover,#eee);color:var(--label,#333);}' +
      '.qhc-confirm{background:var(--accent,#c9a0e0);color:#fff;}' +
